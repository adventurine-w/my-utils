...
public class Common {
    ...
    public static BukkitTask runTaskLater(Runnable runnable, long ticks){
        ...
        return Bukkit.getScheduler().runTaskLater(Clickerflex.getInstance(), runnable, ticks);
    }
    ...
    public static void runTaskLaterS(Runnable runnable, float seconds) {
        //if (seconds <= 0) {
        //    runnable.run();
        //    return;
        //}
        long delayTicks = Math.round(seconds * 20);
        Bukkit.getScheduler().runTaskLater(Clickerflex.getInstance(), runnable, delayTicks);
    }
